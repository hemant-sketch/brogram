...
export default function WorkoutCard(props) {
    const {trainingPlan, workoutIndex, type, dayNum, 
        icon, handleSave, handleComplete, savedWeights} = props;
    const {warmup, workout} = trainingPlan || {};
    const [showExerciseDescription, setShowExerciseDescription] = useState(null)
    const [weights, setWeights] = useState(savedWeights || {});  //woh wale weights jo user input krra
                ...
                {warmup.map((warmupExercise, wIndex) => {
                    return (
                        <React.Fragment key={wIndex}>
                            <div className="exercise-name">
                                <p>{wIndex + 1}. {warmupExercise.name}</p>
                                <button onClick={ ()=> {
